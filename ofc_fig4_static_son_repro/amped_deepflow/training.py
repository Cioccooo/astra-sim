...
import pandas as pd
import perf
from overlap_scheduler import build_time_series_overlap_csv
...
# from astrasim_workload import astrasim_workload
# from mat_dims_mapedToDF import mat_dims_ampedToDF
...
BASE_DIR = Path(__file__).resolve().parent   # 当前这个 .py 文件所在目录
DEEPFLOW_CONFIG_PATH = BASE_DIR / "deepflow_configs"

RESULT_DIR = "DP_2_1_TP_2_1_PP_1_1"
OUTPUT_PATH = BASE_DIR / "output_files" / RESULT_DIR
...
class cal_time:
    ...
    def time_from_GEMM(self):
        ...
        t_elapsed = (
            t_elapsed * 2.0
        )  # FW pass + BW pass (weight update time is added seperately)
    ...
    def main(self, dims):
        ...
        for i in range(len(dims)):
            ...
            if dims[i][4] == "O1.WL2=O2":
                self.deepflow_outputs.append(
                    [mha_GEMMtime, mha_reduction, "MHA", 0, 0, 0]
                )
                self.deepflow_outputs.append(
                    [ffn_GEMMtime, ffn_reduction, "FFN", 0, 0, 0]
                )
...
class astrasim_workload:
    ...
    def astrasim_mmm_breakup(self, B, D, S, h, nheads, h_MLP1, h_MLP2, N_DP, N_PP):
        ...
        print(
            f"Saving astrasim compatible workload file at {OUTPUT_PATH}/{self.amped.timeStamp}mat_dims_astrasim.txt"
        )
        fileName = f"{OUTPUT_PATH}/{self.amped.timeStamp}mat_dims_astrasim.csv"
...
class amped_exec:
    ...
    def training_string_training_time_breakdown(self):
        self.breakdown = self.perf_model.legacy_training_time_breakdown_pairs()
        return self.perf_model.string_training_time_breakdown()

    def training_string_training_time_breakdown_overlap(self):
        return self.perf_model.string_training_time_breakdown_overlap()

    def main(self):
        # Amped inference script
        original_argv = sys.argv[:]
        try:
            # Inputs() parses sys.argv again; keep its view minimal so training.py
            # CLI overrides stay local to this wrapper entrypoint.
            sys.argv = [sys.argv[0]]
            inputs = Inputs()
        finally:
        ...
        if self.training:
            summary += "\n\n" + self.perf_model.string_effective_scheduler_settings()
        ...
        if not self.training:
            ...
            self.temp_save_as(
                ...
                self.training_string_training_time_breakdown_overlap(),
                saveDir=OUTPUT_PATH,
            )
...
def _parse_dp_overlap_mode(value):
    mode_map = {
        "0": 0,
        "1": 1,
        "2": 2,
    ...
    if normalized not in mode_map:
        raise argparse.ArgumentTypeError(
            "dp overlap mode must be one of: 0, 1, 2, lump, layerwise, bucketed"
        )
    return mode_map[normalized]


def _ensure_param_entry(parameters, key, description):
...
def _param_value(parameters, key, default=None):
...
def _dp_mode_name_from_value(raw_mode):
...
def _overlap_mode_subdir(config_data):
    sp = config_data["system_architecture_parameters"]["parameters"]
    dp_mode = _dp_mode_name_from_value(_param_value(sp, "dp_overlap_mode", 0))
    return f"dp_{dp_mode}_pp_off_tp_off"


def _apply_cli_overrides(data, cli_overrides):
    if not cli_overrides:
        return

    sp = data["system_architecture_parameters"]["parameters"]

    if cli_overrides.get("dp_overlap_mode") is not None:
        entry = _ensure_param_entry(
            sp,
            "dp_overlap_mode",
            "DP overlap mode: 0=lump, 1=layerwise, 2=bucketed",
        )
        entry["value"] = int(cli_overrides["dp_overlap_mode"])

    if cli_overrides.get("dp_bucket_size_bytes") is not None:
        entry = _ensure_param_entry(
            sp,
            "dp_bucket_size_bytes",
            "DP bucket size in bytes for bucketed mode; 0 keeps the per-layer minimum",
        )
        entry["value"] = float(cli_overrides["dp_bucket_size_bytes"])

    if cli_overrides.get("ocs_reconf_us") is not None:
        entry = _ensure_param_entry(
            sp,
            "ocs_reconf_per_round",
            "OCS reconfiguration time per round",
        )
        entry["value"] = f"{float(cli_overrides['ocs_reconf_us']) * 1000.0:g} ns"


def _runtime_mirror_path():
    repo_root = os.path.dirname(__file__)
    return os.path.join(repo_root, "amped_backups", "config.json")


def update_configs(cSim, config_file, cli_overrides=None):
    ...
    amped_cfg = _runtime_mirror_path()
    runtime_data = copy.deepcopy(data)
    _apply_cli_overrides(runtime_data, cli_overrides)
    _dump_pretty(amped_cfg, runtime_data)
    return amped_cfg, data, runtime_data
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dp-overlap-mode",
        type=_parse_dp_overlap_mode,
        default=None,
        help="Override DP overlap mode for this run only. Prefer lump/layerwise/bucketed; 0/1/2 are also accepted.",
    )
    parser.add_argument(
        "--dp-bucket-size-bytes",
        type=float,
        default=None,
        help="Override DP bucket size in bytes for this run only. 0 means the per-layer gradient-byte minimum, which yields one layer per bucket in the current model.",
    )
    parser.add_argument(
        "--ocs-reconf-us",
        type=float,
        default=None,
        help="Override OCS reconfiguration time per round in microseconds for this run only.",
    )
    ...
    itrers = [ # GPT3 175B
        "DP_1_16_TP_8_2_PP_1_8_8",   # 2048
    ]
    # itrers = [ # megatron 310B
    #     "DP_1_15_TP_8_2_PP_1_16_8",   # 1920*2
    # ]
    # itrers = [ # megatron 530B
    #     "DP_1_9_TP_8_2_PP_1_35_8",   # 2520*2
    # ]
    # itrers = [ # megatron 1TB
    #     "DP_1_6_TP_8_2_PP_1_64_8",   # 3072*2
    # ]
    ...
    # Figure 4 sanity check: GPT-3 175B SON baseline, 1024 GPUs.
    # Paper label: DP-TP-PP = 16-8-8.
    itrers = [
    ...
    for cSim in itrers:
        ...
        runtime_cfg_path, runtime_base_data, runtime_data = update_configs(
            cSim,
            args.config,
            cli_overrides={
                "dp_overlap_mode": args.dp_overlap_mode,
                "dp_bucket_size_bytes": args.dp_bucket_size_bytes,
                "ocs_reconf_us": args.ocs_reconf_us,
            },
        )
        try:
            RESULT_DIR = cSim
            overlap_dir = _overlap_mode_subdir(runtime_data)
            # OUTPUT_PATH = f"/imec/scratch/dtpatha/patel23/amped_deepflow/amped_deepflow/forNetwork/{RESULT_DIR}"
            OUTPUT_PATH = BASE_DIR / "output_files" / RESULT_DIR / overlap_dir
            ...
            AMPeD.temp_save_as(
                ...
                build_time_series_overlap_csv(
                    AMPeD.inputs,
                    AMPeD.perf_model,
                    DeepFlow.deepflow_outputs,
                ),
                saveDir=OUTPUT_PATH,
            )
            # Time domain list
            ...
            _dump_pretty(runtime_cfg_path, runtime_base_data)
