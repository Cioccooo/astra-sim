...
def validate_config(cfg):
    if cfg["model"]["batch_size"] < cfg["parallelism"]["dp_size"]:
        raise ValueError("batch_size must be >= dp_size")
    batch_size_per_dp = cfg["model"]["batch_size"] // cfg["parallelism"]["dp_size"]
    if batch_size_per_dp < cfg["model"]["num_microbatches"]:
        raise ValueError("batch_size / dp_size must be >= num_microbatches")
